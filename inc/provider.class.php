...
class PluginSinglesignonProvider extends CommonDBTM
{
    ...
    function showForm($ID, $options = [])
    {
        ...
        $on_change =
            'var _value = this.options[this.selectedIndex].value; $(".sso_url").toggle(_value == "generic");';
        ...
        echo "<td><input type='text' style='width:96%' name='client_id' value='" .
            $this->fields['client_id'] .
            "'></td>";
        echo '<td>' . __sso('Client Secret') . '</td>';
        echo "<td><input type='text' style='width:96%' name='client_secret' value='" .
            $this->fields['client_secret'] .
            "'></td>";
        echo "</tr>\n";
        ...
        echo "<td><input type='text' style='width:96%' name='scope' value='" .
            $this->getScope() .
            "'></td>";
        echo '<td>' . __sso('Extra Options') . '</td>';
        echo "<td><input type='text' style='width:96%' name='extra_options' value='" .
            $this->fields['extra_options'] .
            "'></td>";
        echo "</tr>\n";

        echo "<tr class='tab_bg_1 sso_url' $url_style>";
        echo '<td>' . __sso('Authorize URL') . '</td>';
        echo "<td colspan='3'><input type='text' style='width:96%' name='url_authorize' value='" .
        ...
        echo "<td colspan='3'><input type='text' style='width:96%' name='url_access_token' value='" .
        ...
        echo "<td colspan='3'><input type='text' style='width:96%' name='url_resource_owner_details' value='" .
            $this->getResourceOwnerDetailsUrl() .
            "'></td>";
        echo "</tr>\n";
        ...
        echo '</td>';
        echo "<td><input type='text' style='width:96%' name='authorized_domains' value='" .
            $this->fields['authorized_domains'] .
            "'></td>";
        echo "</td></tr>\n";
        ...
        if (!empty($this->fields['picture'])) {
            echo Html::image(
                PluginSinglesignonToolbox::getPictureUrl(
                    $this->fields['picture']
                ),
                [
                    'style' => '
      ...
      $("[name=color]").on("change", function (e) {
        ...
        if ($ID) {
            ...
            $options['addbuttons'] = [
                'test_singlesignon' => __sso('Test Single Sign-on'),
            ];
        }
    ...
    private function prepareInput($input)
    {
        ...
        if ($type === 'generic') {
            if (
                !isset($input['url_authorize']) ||
                empty($input['url_authorize'])
            ) {
                $error_detected[] = __sso('An Authorize URL is required');
            } elseif (
                !filter_var($input['url_authorize'], FILTER_VALIDATE_URL)
            ) {
                $error_detected[] = __sso('The Authorize URL is invalid');
            }

            if (
                !isset($input['url_access_token']) ||
                empty($input['url_access_token'])
            ) {
                $error_detected[] = __sso('An Access Token URL is required');
            } elseif (
                !filter_var($input['url_access_token'], FILTER_VALIDATE_URL)
            ) {
                $error_detected[] = __sso('The Access Token URL is invalid');
            }

            if (
                !isset($input['url_resource_owner_details']) ||
                empty($input['url_resource_owner_details'])
            ) {
                $error_detected[] = __sso(
                    'A Resource Owner Details URL is required'
                );
            } elseif (
                !filter_var(
                    $input['url_resource_owner_details'],
                    FILTER_VALIDATE_URL
                )
            ) {
                $error_detected[] = __sso(
                    'The Resource Owner Details URL is invalid'
                );
            }
        }
        ...
        if (isset($input['_blank_picture']) && $input['_blank_picture']) {
            ...
            if (array_key_exists('picture', $this->fields)) {
                PluginSinglesignonToolbox::deletePicture(
                    $this->fields['picture']
                );
            }
        }
        ...
        if (isset($input['_picture'])) {
            ...
            if (
                $dest = PluginSinglesignonToolbox::savePicture(
                    GLPI_TMP_DIR . '/' . $picture
                )
            ) {
                $input['picture'] = $dest;
            } else {
    ...
    static function getSpecificValueToDisplay(
        $field,
        $values,
        array $options = []
    ) {
    ...
    static function getSpecificValueToSelect(
        $field,
        $name = '',
        $values = '',
        array $options = []
    ...
    function getSpecificMassiveActions($checkitem = null)
    {
        ...
        $actions[
            'Document_Item' . MassiveAction::CLASS_ACTION_SEPARATOR . 'add'
        ] = _x('button', 'Add a document'); // GLPI core one
        $actions[
            __CLASS__ . MassiveAction::CLASS_ACTION_SEPARATOR . 'do_nothing'
        ...
        switch ($ma->getAction()) {
            case 'DoIt':
                if ($item->getType() == 'Computer') {
                    Session::addMessageAfterRedirect(
                        __('Right it is the type I want...', 'example')
                    );
                    Session::addMessageAfterRedirect(
                        __('Write in item history', 'example')
                    );
            ...
            case 'do_nothing':
                if ($item->getType() == 'PluginExampleExample') {
                    ...
                    Session::addMessageAfterRedirect(
                        __('But... I say I will do nothing for:', 'example')
                    );
                    foreach ($ids as $id) {
                        if ($item->getFromDB($id)) {
                            Session::addMessageAfterRedirect(
    ...
    public static function getDefault($type, $key, $default = null)
    {
        if (static::$default === null) {
            $content = file_get_contents(
                dirname(__FILE__) . '/../providers.json'
            );
    ...
    public function getClientSecret()
    {
        ...
        if (
            isset($this->fields['client_secret']) &&
    ...
    public function getAuthorizeUrl()
    {
        ...
        if (
            !isset($fields['url_authorize']) ||
            empty($fields['url_authorize'])
        ) {
    ...
    public function getResourceOwnerDetailsUrl($access_token = null)
    {
        ...
        if (
            !isset($fields['url_resource_owner_details']) ||
            empty($fields['url_resource_owner_details'])
        ) {
            $fields['url_resource_owner_details'] = $value;
        }

        $fields = Plugin::doHookFunction(
            'sso:url_resource_owner_details',
            $fields
        );
        ...
        $url = str_replace(
            '<appsecret_proof>',
            hash_hmac('sha256', $access_token, $this->getClientSecret()),
            $url
        );
    ...
    public function checkAuthorization()
    {
        ...
        if (
            isset($_GET['state']) &&
            is_integer(strpos($_GET['state'], '&redirect='))
        ) {
    ...
    public function getResourceOwner()
    {
        ...
        $headers = Plugin::doHookFunction(
            'sso:resource_owner_header',
            $headers
        );
        ...
        if ($this->getClientType() === 'linkedin') {
            ...
            try {
                ...
                $this->_resource_owner['email-address'] =
    ...
    public function findUser()
    {
        ...
        foreach ($remote_id_fields as $field) {
            if (
                isset($resource_array[$field]) &&
                !empty($resource_array[$field])
            ) {
        ...
        foreach ($login_fields as $field) {
            if (
                isset($resource_array[$field]) &&
                is_string($resource_array[$field])
            ) {
        ...
        if (
            ($login && $user->getFromDBbyName($login)) ||
            ($loginSplited && $user->getFromDBbyName($loginSplited))
        ) {
            return $user;
        }
        ...
        if (
            ($email && $user->getFromDBbyEmail($email, $default_condition)) ||
            ($emailSplited &&
                $user->getFromDBbyEmail($emailSplited, $default_condition))
        ) {
            return $user;
        } else {
            $bOk = false;
        ...
        if (
            static::getClientType() == 'generic' &&
            !$bOk &&
            $this->fields['add_user']
        ) {
            try {
                // Generates an api token and a personal token
                $tokenAPI = base_convert(
                    hash('sha256', time() . mt_rand()),
                    16,
                    36
                );
                $tokenPersonnel = base_convert(
                    hash('sha256', time() . mt_rand()),
                    16,
                    36
                );
                ...
                foreach ($login_fields as $field) {
                    if (
                        ...
                        $userPost['realname'] = preg_split(
                            '/ /',
                            $resource_array['displayName']
                        )[1];
                        $userPost['firstname'] = preg_split(
                            '/ /',
                            $resource_array['displayName']
                        )[0];
                        break;
                ...
                if (0 == Profile::getDefault()) {
                    ...
                    if (
                        count($datasProfiles) > 0 &&
                        count($datasEntities) > 0
                    ) {
