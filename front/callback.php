...
if ($user_id || $signon_provider->login()) {
    ...
    if (isset($_SESSION['redirect'])) {
        $REDIRECT = '?redirect=' . $_SESSION['redirect'];
    } elseif (
        isset($_GET['state']) &&
        ...
        $REDIRECT =
            '?' .
            substr($_GET['state'], strpos($_GET['state'], '&redirect=') + 1);
    }
    ...
    if ($_SESSION['glpiactiveprofile']['interface'] == 'helpdesk') {
        if (
            ...
            $url_redirect =
                $CFG_GLPI['root_doc'] . "/front/helpdesk.public.php$REDIRECT";
        }
    } else {
        if (
            $_SESSION['glpiactiveprofile']['create_ticket_on_login'] &&
            ...
            $url_redirect =
                $CFG_GLPI['root_doc'] . "/front/central.php$REDIRECT";
        }
    }

    Html::nullHeader('Login', $CFG_GLPI['root_doc'] . '/index.php');
    echo '<div class="center spaced"><a href="' .
        $url_redirect .
        '">' .
        __('Automatic redirection, else click') .
        '</a>';
